...
const JobFlow = () => {
  ...
  const handleSubmitReport = async () => {
    if (!reportForm.summary.trim()) { setError('Please provide a summary'); return; }
                  ...
                  <div style={{ marginTop: 'var(--space-lg)' }}>
                    {!showReportForm ? <button onClick={() => setShowReportForm(true)} className={styles.primaryButton}>Proceed to Submit Report</button> : (
                      <div className={styles.reportForm}>
                        ...
                        <button onClick={handleSubmitReport} disabled={actionLoading || !reportForm.summary.trim()} className={styles.primaryButton}>{actionLoading ? <><div className={styles.spinner} />Submitting...</> : 'Submit Inspection Report'}</button>
                        <button onClick={() => navigate('/dashboard')} className={styles.secondaryButton} style={{ marginTop: 'var(--space-md)' }}>Back to Dashboard</button>
                      </div>
                    )}
                  </div>
                )}
